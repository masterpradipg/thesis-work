...
import numpy as np
import thesis_qubo
from thesis_qubo.common.dense_graph_annealer_base import DenseGraphAnnealerBase
...
def dense_graph_annealer(W = None, optimize=thesis_qubo.minimize, dtype=np.float64, **prefs) :
    ann = DenseGraphAnnealer(W, optimize, dtype, prefs)
    return ann

# inherit docstring from interface
docstring.inherit(DenseGraphAnnealer, thesis_qubo.py.DenseGraphAnnealer)
...
if __name__ == '__main__' :
    ...
    N = 100
    m = 5
    W = thesis_qubo.generate_random_symmetric_W(N, -0.5, 0.5, np.float64)

    ann = dense_graph_annealer(W, dtype=np.float64, n_trotters = m)
    import thesis_qubo.cuda as py
    #ann = py.dense_graph_annealer(W, n_trotters = m)
    ann.set_qubo(W, thesis_qubo.minimize)
